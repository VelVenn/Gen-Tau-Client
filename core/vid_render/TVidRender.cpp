...
TVidRender::TVidRender(const char* file_path) : naluBuffer(0)
{
	...
	if constexpr (conf::TDebugMode) {
		...
		gst_bin_add_many(GST_BIN(pipeline), src, parser, decoder, vconv, conv, sink, NULL);
	} else {
		throw std::runtime_error("Calling TVidRender(const char* file_path) is not supported in release builds.");
	}
}
}  // namespace gentau